...
import { X, Monitor, Smartphone, Square } from "lucide-react";
...
interface NewDemoDialogProps {
  appId: string;
  onClose: () => void;
  onCreate: (name: string, format: DemoFormat) => void;
}

const formatOptions: { format: DemoFormat; icon: typeof Monitor; label: string; dimensions: string }[] = [
  { format: "youtube", icon: Monitor, label: "YouTube", dimensions: "1920 × 1080" },
  { format: "youtube_4k", icon: Monitor, label: "YouTube 4K", dimensions: "3840 × 2160" },
  { format: "tiktok", icon: Smartphone, label: "TikTok / Reels", dimensions: "1080 × 1920" },
  { format: "square", icon: Square, label: "Square", dimensions: "1080 × 1080" },
];

export function NewDemoDialog({ appId, onClose, onCreate }: NewDemoDialogProps) {
  const [name, setName] = useState("My App Demo");
  const [selectedFormat, setSelectedFormat] = useState<DemoFormat>("youtube");

  const handleCreate = () => {
    if (!name.trim()) return;
    onCreate(name.trim(), selectedFormat);
  };
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-default)]">
          <h3 className="font-semibold text-[var(--text-primary)]">New Demo Project</h3>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {formatOptions.map(({ format, icon: Icon, label, dimensions }) => (
                <button
                  key={format}
